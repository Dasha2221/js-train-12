...
function sortByValues(dictionary) {
  ...
  return sortedDictionary; // Повертаємо відсортований словник
}
...
function convertDictionaryToSet(dictionary) {
  ...
  // Перебираємо ключі словника разом з їх значеннями за допомогою циклу for...of
  for (const [key, value] of dictionary.entries()) {
  ...
  return set; // Повертаємо модифіковану множину
...
function convertSetToDictionary(set) {
  ...
  for (const elem of set) {
    const firstCharCode = elem.charCodeAt(0);
    map.set(elem, firstCharCode);
  }
  return map;
